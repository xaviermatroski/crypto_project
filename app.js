...
app.post("/login", async (req, res) => {
  try {
    ...
    if (user.isBlocked) {
      return res.render("login", {
        error: `Account is blocked. Reason: ${
          user.blockedReason || "Contact administrator"
        }`,
      });
    }
    ...
    if (user.jurisdiction && user.jurisdiction.district === "DistrictPoliceA") {
      ...
      user.jurisdiction &&
      user.jurisdiction.district === "DistrictPoliceB"
    ) {
          ...
          $project: {
            role: {
              $switch: {
                branches: [
                  {
                    case: { $eq: ["$_id", "investigator"] },
                    then: "Investigators",
                  },
                  {
                    case: { $eq: ["$_id", "forensics_officer"] },
                    then: "Forensics Officers",
                  },
...
app.get("/admin/users", async (req, res) => {
  if (req.session.userId && req.session.userRole === "admin") {
    try {
      ...
      const jurisdictions = await Jurisdiction.find({}).sort({
        state: 1,
        district: 1,
      });
  ...
  if (req.session.userId && req.session.userRole === "admin") {
    try {
      ...
      if (user.role === "judiciary") {
        // Set jurisdiction level for judiciary users
        updateData.jurisdictionLevel = req.body.jurisdictionLevel || "district"; // default to district
        if (req.body.jurisdiction) {
          updateData.jurisdiction =
            typeof req.body.jurisdiction === "string"
              ? JSON.parse(req.body.jurisdiction)
              : req.body.jurisdiction;
        }
      } else if (req.body.jurisdiction) {
        // Handle other roles' jurisdiction
        updateData.jurisdiction =
          typeof req.body.jurisdiction === "string"
            ? JSON.parse(req.body.jurisdiction)
            : req.body.jurisdiction;
...
app.post("/admin/policies/add", isAdmin, async (req, res) => {
  try {
    ...
    const policyDataObj = JSON.parse(policyData);

    // Create policy in MongoDB
    const policy = new Policy({
      policyId: policyDataObj.policyId,
      name,
      description,
      categories: policyDataObj.categories,
      rules: policyDataObj.rules,
      createdBy: req.session.userId,
    });

    await policy.save();

    // Create policy on blockchain with correct format
    // Create policy on blockchain with proper stringified format
    try {
      await createPolicyOnBlockchain(
        policyDataObj.policyId?.toString(),
        policyDataObj.categories || [], // Pass the object/array directly
        policyDataObj.rules || [],      // Pass the object/array directly
        req.session.orgMspId?.toString()
      );
